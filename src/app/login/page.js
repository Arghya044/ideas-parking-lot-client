...
import { signIn, register, signInWithGoogle } from '@/lib/auth';
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.error) {
          ...
          } else if (response.error.includes('auth/email-already-in-use')) {
            errorMessage = 'An account with this email already exists.';
          } else if (response.error.includes('auth/weak-password')) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Use the error message from the auth function (already user-friendly)
        const errorMessage = response.error || 'Failed to sign in with Google. Please try again.';
        setMessage(errorMessage);
      }
    } catch (error) {
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">
              {isRegister
                ? 'Sign up to start parking your ideas'
          ...
          <button
            ...
            className="w-full mb-6 px-4 py-3 bg-white border-2 border-gray-300 rounded-lg hover:border-gray-400 transition-colors flex items-center justify-center space-x-2 font-medium text-gray-700"
          >
